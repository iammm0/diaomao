import { motion, type Easing, type TargetAndTransition, type Transition } from 'motion/react'
...
const buildKeyframes = (
  ...
  keys.forEach((key) => {
    keyframes[key] = [from[key] ?? 0, ...steps.map((step) => step[key] ?? 0)]
  })
  return keyframes
}
...
export default function BlurText({
  ...
  useEffect(() => {
    ...
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || isInViewport(node)) {
      reveal()
      return
    }
